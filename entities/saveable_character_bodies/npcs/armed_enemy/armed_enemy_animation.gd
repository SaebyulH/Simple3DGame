...
@onready var npc: NonPlayerCharacter = get_owner()
...
func _physics_process(delta: float) -> void:
	...
	"not_aiming" if npc.inventory_data.current_index == -1 else "aiming")
	
	#print(animation_tree.get("parameters/aim_transition/current_state"))
	# Convert npc velocity to skin's local space
	var local_velocity = global_transform.basis.inverse() * npc.velocity
	...
	animation_tree.set("parameters/idle_walk_blend/blend_amount", smoothed_velocity.length()/npc.character_data.speed)
